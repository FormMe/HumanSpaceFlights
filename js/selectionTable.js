class SelectionTable{
	constructor(){
		...
		this.margin = {top: 66, right: 110, bottom: 20, left: 188},
		...
		var container = d3.select("#parallelCoords")
		    .attr("class", "view parcoords")
		...
		var output = container.append("pre");
	...
	update(data){
		...
		var container = d3.select("#parallelCoords");
		var svg = container.select("svg").select('g');
		var canvas = container.select("canvas");
		// var output = container.select("pre");
		...
		d3.csv("data/planets.csv", function(error, data) {
